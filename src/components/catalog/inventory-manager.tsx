  ...
  return (
    <article className={`rounded-xl border bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.03)] ${item.isLowStock ? "border-amber-300" : "border-slate-200"}`}>
      <div className="flex items-start justify-between gap-4">
        ...
        <form action={deleteAction}><input type="hidden" name="ingredientId" value={item.ingredientId} /><ConfirmSubmitButton pending={deleting} label="Delete" confirmLabel="Delete ingredient" message={`Delete ${item.name}? This cannot be undone.`} className="rounded-lg border border-rose-200 px-3 py-2 text-xs font-semibold text-rose-600 hover:bg-rose-50 disabled:opacity-50" /></form>
      </div>
