...
export function PurchaseOrderProposalCard({ proposal, currency }: { proposal: PurchaseOrderAIActionProposal; currency: string }) {
  ...
  const detail = result?.success && result.reference ? <> <Link className="font-bold underline" href="/purchase-orders">View Purchase Order</Link></> : null;
  return <AIActionCard proposal={proposal} status={status} result={result ? { success: result.success, message: result.message, detail } : null} actions={actions}>
    <div className="mt-4 rounded-lg bg-slate-50 px-3 py-2 text-sm"><span className="text-slate-500">Reference: </span><strong>assigned after approval</strong></div><div className="mt-3 flex justify-between border-b border-slate-100 pb-2 text-sm"><span className="text-slate-500">Supplier</span><strong>{proposal.display.supplierName}</strong></div>
