...
type DisplayMessage = AIConversationMessage & { toolsUsed?: string[]; error?: boolean; clearHistory?: boolean };
...
export function AIManagerWorkspace({ restaurantName }: { restaurantName: string }) {
  ...
  function submit(event?: FormEvent, suggested?: string) {
    ...
    setActivityIndex(0);
    const history = buildBrowserConversationHistory(messages);
    ...
    startTransition(async () => {
      ...
      const reply: DisplayMessage = result.success ? { role: "assistant", content: result.answer, toolsUsed: result.toolsUsed } : { role: "assistant", content: result.message, error: true, clearHistory: result.clearHistory };
